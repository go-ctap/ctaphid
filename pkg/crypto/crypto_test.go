...
func TestLargeBlobRejectsInvalidKeyLength(t *testing.T) {
	...
	_, err = DecryptLargeBlob(shortKey, ctaptypes.LargeBlob{})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "large blob key length")
}
...
func TestPinUvAuthProtocolEncapsulateAndEncryptDecrypt(t *testing.T) {
	for _, tc := range []struct {
		name            string
		protocol        ctaptypes.PinUvAuthProtocol
		sharedSecretLen int
		ciphertextLen   int
	}{
		{name: "protocol one", protocol: ctaptypes.PinUvAuthProtocolOne, sharedSecretLen: 32, ciphertextLen: 16},
		{name: "protocol two", protocol: ctaptypes.PinUvAuthProtocolTwo, sharedSecretLen: 64, ciphertextLen: 32},
	} {
...
func TestAuthenticateDispatchesByProtocol(t *testing.T) {
	...
	protocolOneMAC := Authenticate(ctaptypes.PinUvAuthProtocolOne, sharedSecret32, message)
	assert.Len(t, protocolOneMAC, 16)

	protocolTwoMAC := Authenticate(ctaptypes.PinUvAuthProtocolTwo, sharedSecret64, message)
	assert.Len(t, protocolTwoMAC, 32)
	assert.Equal(t, Authenticate(ctaptypes.PinUvAuthProtocolTwo, sharedSecret32, message), protocolTwoMAC)
}
...
func encryptLargeBlobWithOrigSize(t *testing.T, key []byte, origData []byte, origSize uint) ctaptypes.LargeBlob {
	t.Helper()
